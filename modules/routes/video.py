    ...
    def run_worker():
        try:
            ...
            if yt_url:
                ...
                subprocess.run(["yt-dlp", "-f", "best", "-o", dl_path, yt_url], check=True)
                video_path = dl_path
                state.active_project.video_path = video_path
                state.active_project.save()
            ...
            if subtitles_path and os.path.exists(subtitles_path):
                ...
                speakers = ["Unknown"] * len(source_texts)
                state.active_project.source_lang = [k for k, v in LANGUAGE_MAPPING.items() if v == detected][0] if detected in LANGUAGE_MAPPING.values() else detected
